# Based on ...
# Colors: for code ({000..255}) { print -nP -- "$code: %F{$code}%K{$code}Test%k%f " ; (( code % 8 && code < 255 )) || printf '\n'}
# Changes marked with "ORIGINAL"
#
# Pure
# by Sindre Sorhus
...
prompt_pure_preprompt_render() {
	...
	# Set the path.
	# ORIGINAL: preprompt_parts+=('%F{${prompt_pure_colors[path]}}%~%f')
	preprompt_parts+=('%F{002}%~%f')

	# Git branch and dirty status info.
	typeset -gA prompt_pure_vcs_info
	if [[ -n $prompt_pure_vcs_info[branch] ]]; then
		# ORIGINAL: preprompt_parts+=("%F{$git_color}"'${prompt_pure_vcs_info[branch]}'"%F{$git_dirty_color}"'${prompt_pure_git_dirty}%f')
	...
	local -ah ps1
	ps1=(
    # ORIGINAL; ${(j. .)preprompt_parts}  # Join parts, space separated.
      ...
      if [[ -n $output ]]; then
		...
		prompt_pure_async_git_dirty)
			...
			if (( code == 0 )); then
				...
				# ORIGINAL: typeset -g prompt_pure_git_dirty="*"
				typeset -g prompt_pure_git_dirty="%F{001}✘ %f"
			fi
